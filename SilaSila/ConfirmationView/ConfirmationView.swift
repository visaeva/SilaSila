...
struct ConfirmationView: View {
	...
	private var selectedBaseView: some View {
		HStack {
	...
	private var participiantView: some View {
		let validParticipants = participants.filter { !$0.name.isEmpty && !$0.lastName.isEmpty && !$0.email.isEmpty }
		
		return 	ForEach(Array(validParticipants.enumerated()), id: \.element.id) { index, participant in
			VStack(alignment: .leading) {
